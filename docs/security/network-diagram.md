...
- Outbound AI request to OpenRouter using an OpenAI model
...
flowchart LR
    ...
    A -->|HTTPS API request with OPENROUTER_API_KEY| O
...
- The backend sends prompt content derived from those answers and notes to OpenRouter for the configured OpenAI model.
...
- The only required secret currently identified in code is `OPENROUTER_API_KEY` from [services.py](/Users/devanshijain/SBDC/services.py:38).
